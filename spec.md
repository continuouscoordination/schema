...
A **membership** connects a person to a team. It is the source of truth for who belongs to which team. A person can have memberships in multiple teams. Memberships can carry per-team attributes like role and team check-in cadence.
...
A team check-in is associated with one or more memberships. Each membership encodes both the person and the team, so the team check-in inherits both relationships. This ensures a person can only check in to teams they belong to. When work spans teams, a single team check-in can reference multiple memberships rather than requiring duplicates.
...
person --authors--> goal-story-update (one-to-many)
coordination-event --references--> team-checkin | goal-story-update (one-to-one)
person --triggers--> coordination-event (one-to-many)
```
